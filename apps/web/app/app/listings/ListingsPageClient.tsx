...
type OffM = ListingsPageData["offMarket"][number];
...
export default function ListingsPageClient({
  ...
  const tabLabels = useMemo(
    () =>
      [
        { id: "active" as const, label: "Active", section: "i", count: data.activeListings.length },
        { id: "drafts" as const, label: "Drafts", section: "ii", count: data.draftListings.length },
        {
          id: "off-market" as const,
          label: "Off-market",
          section: "iii",
          count: data.offMarket.length,
        },
        {
          id: "archive" as const,
          label: "Archive",
          section: "iv",
          count: data.archiveListings.length,
        },
      ] satisfies { id: Tab; label: string; section: string; count: number }[],
    [data],
  );

  const totalViews = data.performance.reduce((s, r) => s + r.views7d, 0);
  const totalLeads = data.performance.reduce((s, r) => s + r.enquiries7d, 0);
      ...
      <main className="dash">
        ...
        <section className="dash-kpis" aria-label="Portfolio metrics">
          <KpiTile
            ...
            value={data.activeListings.length}
          ...
          <KpiTile
            ...
            value={data.authorityExpiringSoon.length}
        ...
        {tab === "active" ? <ActivePanel activeListings={data.activeListings} /> : null}
        {tab === "drafts" ? <DraftsPanel draftListings={data.draftListings} /> : null}
        {tab === "off-market" ? <OffMarketPanel offMarket={data.offMarket} /> : null}
        {tab === "archive" ? <ArchivePanel archiveListings={data.archiveListings} /> : null}
...
function ActivePanel({ activeListings }: { activeListings: Listing[] }) {
  return (
...
function DraftsPanel({ draftListings }: { draftListings: DraftListing[] }) {
  return (
    <section className="dash-row">
      <article className="dash-panel">
        <header className="dash-panel-head">
...
function OffMarketPanel({ offMarket }: { offMarket: OffM[] }) {
  return (
    <section className="dash-row">
      <article className="dash-panel">
        <header className="dash-panel-head">
