...
function matchesFilter(f: FilterId, item: Listing) {
  ...
  if (f === "week")
    return !item.isExpired && item.daysLeft != null && item.daysLeft <= 7;
  if (f === "month") {
    return (
      !item.isExpired &&
      item.daysLeft != null &&
...
export function AuthorityExpiringScreen({ navigation }: Props) {
  ...
  const list = useMemo(
    () => ALL.filter((i) => matchesFilter(filter, i)),
    [filter],
  );
  ...
  return (
    <View style={styles.root}>
      ...
      <SafeAreaView style={styles.safe} edges={["top"]}>
        <TopBar title="Authority expiring" />
        <ScrollView
          horizontal
        ...
        {list.map((l) => (
          <View key={l.id} style={styles.card}>
            ...
            <Text style={styles.actionLine}>
              RENEW AUTHORITY · CONTACT VENDOR
            </Text>
          </View>
        ))}
...
const styles = StyleSheet.create({
  ...
  chipText: { fontSize: 12, fontWeight: "500", color: brand.charcoal },
  ...
  subL: { fontSize: 11, color: brand.sage, fontWeight: "500" },
  subR: {
    fontSize: 10,
    color: brand.sage,
    letterSpacing: 0.2,
  ...
  cardName: { flex: 1, fontSize: 16, fontWeight: "500", color: brand.charcoal },
  statusPill: {
